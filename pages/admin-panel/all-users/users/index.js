...
import {Backdrop, Checkbox, CircularProgress, TableCell, TextField, Typography} from "@material-ui/core"
...
function Users() {
    ...
    const handleSelectAllClick = (event) => {
        if (event?.target?.checked) {
            if (searchValue && searchValue !== "") {
                setSelectedRows(searchedRows?.map((v) => v.email))
            } else {
                API()
                    .get(
                        `users/list?getAllEmails=${true}`,
                        {
                            headers: {
                    ...
                    .then((response) => {
                        ...
                        if ((response.status = 200)) {
                            setSelectedRows(response?.data?.map((v) => v.email))
                        }
                    })
                    .catch((e) => {
                        setLoading(false)
                        console.log("get all selected users error: ", e)
                    })
            }
            // setSelectedRows(searchedRows?.map((v) => v.email));
    ...
    const openEmailsPage = () => {
        globalDispatch({type: "SET_ADMIN_USER_EMAILS", payload: selectedRows})
        router.push("/admin-panel/all-users/users/mail")
    }
                        ...
                        <Grid container spacing={10}>
                            <Grid style={{display: "flex"}} item xs={6}>
                                <SearchBar
                                    ...
                                    size="small"
                                    onChange={(searchVal) => { setSearchValue(searchVal); setSelectedRows([]); globalDispatch({type: "SET_ADMIN_USER_EMAILS", payload: []}) }}
