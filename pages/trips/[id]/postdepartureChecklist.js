    ...
    return (
        <>
            ...
            <PreDepartureContainer isPreDeparture={false} header={t.postdepartureChecklist.header} imgSource={"/assets/images/undraw_Weather_re_qsmd_1.png"}
                ...
                onNextClick={() => { router.push(`/trips/${  router.query?.id  }/endTripCode`) }}
                onReportClick={() => { setReportSelected(true) }}
                onHintClick={() => { }}>
                <ul style={{ paddingLeft: "20px" }}>
                    <li className={classes.text}>{t.postdepartureChecklist.p1}</li>
                </ul>
                <p className={classes.text}>{t.postdepartureChecklist.p2}</p>
                <ul style={{ paddingLeft: "20px" }}>
                    <li className={classes.text}>{t.postdepartureChecklist.p3}</li>
                </ul>
                <p className={classes.text}>
                    {t.postdepartureChecklist.p4}
                    <span className={classes.reviewLink} onClick={() => setReviewOpen(true)}>
                        {t.postdepartureChecklist.p5}
                    </span>
                </p>
